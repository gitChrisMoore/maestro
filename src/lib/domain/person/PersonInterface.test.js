...
beforeAll(async () => {
    const { session,error } = await supabase.auth.signIn({ email: MOCK_USER_1.email, password:  MOCK_USER_1.password })
...
describe('Person Integration Test', () => {
    ...
    it('Delete all persons from the user', async () => {
        const newPerson2 = await createPerson(MOCK_USER_1);
        const newPerson3 = await createPerson(MOCK_USER_1);
        let allPersons = await getPersons();
        expect(allPersons.length).toBeGreaterThan(0)
